...
function drawCircle2(myCircle3, context2) {
  context2.beginPath();
  context2.arc(myCircle3.x, myCircle3.y, myCircle3.mass, 0, 2 * Math.PI, false);
...
function circle2(myCircle3, context2){
  mu      =  1+m1/m2;
  d2Phi1  =  (g*(Math.sin(Phi2)*Math.cos(Phi1-Phi2)-mu*Math.sin(Phi1))-(l2*dPhi2*dPhi2+l1*dPhi1*dPhi1*Math.cos(Phi1-Phi2))*Math.sin(Phi1-Phi2))/(l1*(mu-Math.cos(Phi1-Phi2)*Math.cos(Phi1-Phi2)));
  d2Phi2  =  (mu*g*(Math.sin(Phi1)*Math.cos(Phi1-Phi2)-Math.sin(Phi2))+(mu*l1*dPhi1*dPhi1+l2*dPhi2*dPhi2*Math.cos(Phi1-Phi2))*Math.sin(Phi1-Phi2))/(l2*(mu-Math.cos(Phi1-Phi2)*Math.cos(Phi1-Phi2)));
  dPhi1   += d2Phi1*time;
  ...
  myCircle3.x = X0+l1*Math.sin(Phi1)+l2*Math.sin(Phi2);
  myCircle3.y = Y0+l1*Math.cos(Phi1)+l2*Math.cos(Phi2);
  
  drawCircle2(myCircle3, context2);
}
...
var canvas2 = document.getElementById('ball2');
...
function run(){
  ...
  var myCircle3 = {x: X0+l1*Math.sin(Phi1)+l2*Math.sin(Phi2), y: Y0+l1*Math.cos(Phi1)+l2*Math.cos(Phi2), mass: m2};
